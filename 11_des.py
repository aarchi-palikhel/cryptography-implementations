import binascii

# Initial and Final Permutation Tables
IP = [58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4,
      62, 54, 46, 38, 30, 22, 14, 6, 64, 56, 48, 40, 32, 24, 16, 8,
      57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3,
      61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7]

FP = [40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31,
      38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29,
      36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27,
      34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25]

def permute(block, table):
    return ''.join(block[i - 1] for i in table)

def xor(bits1, bits2):
    return ''.join(str(int(b1) ^ int(b2)) for b1, b2 in zip(bits1, bits2))

def feistel(right, subkey):
    return xor(right, subkey[:32])

def des_encrypt(plaintext, key):
    plaintext = bin(int.from_bytes(plaintext.encode(), 'big'))[2:].zfill(64)
    key = bin(int.from_bytes(key.encode(), 'big'))[2:].zfill(64)
    
    permuted_text = permute(plaintext, IP)
    L, R = permuted_text[:32], permuted_text[32:]
    
    for _ in range(16):
        temp = R
        R = xor(L, feistel(R, key))
        L = temp
    
    encrypted_text = permute(L + R, FP)
    return binascii.hexlify(int(encrypted_text, 2).to_bytes(8, 'big')).decode()

def des_decrypt(ciphertext, key):
    ciphertext = bin(int(ciphertext, 16))[2:].zfill(64)
    key = bin(int.from_bytes(key.encode(), 'big'))[2:].zfill(64)
    
    permuted_text = permute(ciphertext, IP)
    L, R = permuted_text[:32], permuted_text[32:]
    
    for _ in range(16):
        temp = L
        L = xor(R, feistel(L, key))
        R = temp
    
    decrypted_text = permute(L + R, FP)
    return int(decrypted_text, 2).to_baytes(8, 'big').decode().strip()

# Input from user
plaintext = input("Enter plaintext (8 characters): ")
key = input("Enter 8-character key: ")

# Encrypt and Decrypt
ciphertext = des_encrypt(plaintext, key)
decrypted_text = des_decrypt(ciphertext, key)

print(f"Ciphertext: {ciphertext}")
print(f"Decrypted text: {decrypted_text}")