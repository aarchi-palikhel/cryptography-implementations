...
# Day 14 of 60 Days Learning Challenge
P = [
    16, 7, 20, 21,
    29, 12, 28, 17,
    1, 15, 23, 26,
...
def unpad(data):
    pad_len = data[-1]
    return data[:-pad_len]
def des_encrypt(plaintext, key_bytes):
    key_bits = string_to_bitlist(key_bytes)
    subkeys = generate_subkeys(key_bits)
    padded = pad(plaintext)
    ciphertext = b''
    for i in range(0, len(padded), 8):
        block = string_to_bitlist(padded[i:i+8])
        enc_block = des_encrypt_block(block, subkeys)
        ciphertext += bitlist_to_bytes(enc_block)
    return ciphertext
def des_decrypt(ciphertext, key_bytes):
    key_bits = string_to_bitlist(key_bytes)
    subkeys = generate_subkeys(key_bits)
    decrypted = b''
    for i in range(0, len(ciphertext), 8):
        block = string_to_bitlist(ciphertext[i:i+8])
        dec_block = des_decrypt_block(block, subkeys)
        decrypted += bitlist_to_bytes(dec_block)
